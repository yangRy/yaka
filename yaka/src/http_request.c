...
static int yk_http_process_ignore(yk_http_request_t *r, yk_http_out_t *out, char *data, int len);
static int yk_http_process_connection(yk_http_request_t *r, yk_http_out_t *out, char *data, int len);
static int yk_http_process_if_modified_since(yk_http_request_t *r, yk_http_out_t *out, char *data, int len);

yk_http_header_handle_t yk_http_headers_in[] = {
    {"Host", yk_http_process_ignore},
    {"Connection", yk_http_process_connection},
    {"If-Modified-Since", yk_http_process_if_modified_since},
    {"", yk_http_process_ignore}
};
...
void yk_http_handle_header(yk_http_request_t *r, yk_http_out_t *o) {
    list_head *pos;
    yk_http_header_t *hd;
    yk_http_header_handle_t *header_in;
    int len;
    list_for_each(pos, &(r->list)) {
//    	printf("addr: %d\n", (size_t)&((yk_http_header_t *)0)->list);
        hd = list_entry(pos, yk_http_header_t, list); //这么一堆宏定义，其实就是用结构体的成员找到整个结构体的地址
        /* handle */
        for (header_in = yk_http_headers_in; 

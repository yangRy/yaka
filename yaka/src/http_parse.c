...
/*
 * yk_http_parse_request_line和yk_http_parse_request_body中的状态空间在数值上是一致的，这样就会有bug
 * 当解析到body，read返回，再次进入do_request这个函数时，会再次接卸request_line，这肯定导致错误
 * */
...
int yk_http_parse_request_line(yk_http_request_t *r) {
    ...
    enum {
        ...
        sw_http, //url结束后 HTTP开始之前的状态
        sw_http_H,
        sw_http_HT,
        sw_http_HTT,
        sw_http_HTTP,
        sw_first_major_digit, //开始解析主版本号的状态
        sw_major_digit,
    ...
    for (pi = r->pos; pi < r->last; pi++) {
        p = (u_char *)&r->buf[pi % MAX_BUF]; //p是指针
        ch = *p;
        ...
        switch (state) {
			...
			case sw_method:
				if (ch == ' ') {
					...
					case 4:
						if (yk_str3Ocmp(m, 'P', 'O', 'S', 'T')) {
							r->method = ZV_HTTP_POST;
							break;
						}

						if (yk_str4cmp(m, 'H', 'E', 'A', 'D')) {
    ...
    r->state = REQUESTBODYBASE; //added by AndreYang
...
//key1:value1\r\nkey2:value2\r\n\r\n
    enum {
        ...
        sw_cr,
        sw_crlf,
        sw_crlfcr //headers与requestbody之间有一个空行
    } state;
  ...
  //  check(state == 0, "state should be 0");
        ...
        switch (state) {
			...
			case sw_cr:
				if (ch == LF) {
					...
					hd = (yk_http_header_t *)malloc(sizeof(yk_http_header_t));
    ...
    r->state = REQUESTLINEBASE; //added by AndreYang
